...
<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-6">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6">Edit Visit</h1>
    <form action="{{ route('admin.visits.update', $visit->id) }}" method="POST" class="bg-white shadow rounded-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="notes" id="notes" rows="3" placeholder="Enter any additional notes"
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $visit->notes }}</textarea>
        </div>
